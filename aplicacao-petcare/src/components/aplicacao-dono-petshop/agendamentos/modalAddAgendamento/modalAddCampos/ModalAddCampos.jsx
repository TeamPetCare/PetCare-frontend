...
const ModalAddCampos = ({
  ...
  const handlePaymentStatusChange = (event) => {
    const value = event.target.value; // Obtém o valor diretamente
    setPaymentStatus(value); // Atualiza o estado com o valor selecionado
    checkFormCompletion(); // Checa se o formulário está completo
  };
      ...
      <div className={styles["row1"]}>
        <div className={styles["dropdown-container"]}>
          ...
          {searchTerm.length >= 3 && filteredClients.length > 0 && (
            <ul
              className={`${styles["dropdown-list"]} ${searchTerm.length < 3 || filteredClients.length === 0
                  ? styles["hidden"]
                  : ""
                }`}
            >
